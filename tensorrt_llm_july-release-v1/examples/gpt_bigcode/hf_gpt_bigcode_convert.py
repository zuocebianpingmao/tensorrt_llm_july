'''
Convert huggingface starcoderbase-7b model. Use https://huggingface.co/bigcode/starcoderbase-7b as demo.
'''
import argparse
import configparser
import dataclasses
...
def smooth_gpt_model(model, scales, alpha):
    ...
    for name, module in model.named_modules():
        if not isinstance(module, GPTBigCodeBlock):
            continue
...
@torch.no_grad()
def hf_gpt_bigcode_converter(args: ProgArgs):
    ...
    for key in vars(args):
        config["gpt_bigcode"][key] = f"{vars(args)[key]}"
    for k, v in vars(model.config).items():
        config["gpt_bigcode"][k] = f"{v}"
    config["gpt_bigcode"]["storage_dtype"] = args.storage_type
    config["gpt_bigcode"]["multi_query_mode"] = str(multi_query_mode)
